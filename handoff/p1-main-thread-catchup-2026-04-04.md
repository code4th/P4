...
- `openclaw agent --agent p1` の direct route を external-core front door に確実につなぐ薄い adapter
...
「P1 の外部コアは成立したが、OpenClaw に registered agent として載せただけではまだ embedded route で走る。次は OpenClaw 上でメインとは別の P1 agent interface を正面入口として実装し、その direct route を external core front door へ確実につなぐ。」
