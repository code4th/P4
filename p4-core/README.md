...
P4 discovers local models from Ollama (`/api/tags`, equivalent to `ollama list`) and uses configured defaults only as a fallback. The dashboard model selector shows the live Ollama model names, and CLI turns can pin a visible model with `--model`.
...
python3 -m p4_core.cli --root /tmp/p4-demo ollama-status
python3 -m p4_core.cli --root /tmp/p4-demo chat --run-immediately --model qwen3.6:latest --message "このworkspaceで最初に確認すべきファイルを読んで"
...
Default model routing separates broad reasoning from code production: `reasoning` uses `gemma4:26b`, `coding` uses `qwen3-coder`, `terminal` uses `qwen3-coder`, and `fast` uses `glm-4.7-flash`. Default Ollama output budgets are intentionally bounded but large enough for JSON tool calls: `coding` uses `num_predict=8192`, `terminal` uses `4096`, `reasoning` uses `1024`, and `fast` stays at `384`. A `done_reason` of `length` means the model hit this budget before completing the required JSON envelope.

`runtime.max_steps_per_message` is a safety valve, not a completion contract. Requests that explicitly combine implementation and unittest evidence use `runtime.verified_implementation_max_steps` (default `32`) as a larger bounded budget so recovery steps do not consume the whole turn before tests can be written and run. If the step limit is still reached, P4 logs `system_note.code = step_limit_reached` with `contract_state` and `missing_requirements`.
