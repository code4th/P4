...
def _controller_terminal_finish(
    ...
    *,
    ...
    wants_observable_execution = any(
        marker in str(user_message or "").lower()
        for marker in ["実行", "run", "execute", "表示", "display", "show", "出力"]
    )
    if str(selection.get("role") or "") != "terminal" and not wants_observable_execution:
        return None
...
def _deterministic_terminal_final_answer(self, *, goal_text: str, user_message: str, steps: list[dict[str, Any]]) -> str | None:
    ...
    for row in evidence[-4:]:
        ...
        if bool(row.get("ok")):
            if stdout.strip():
                if not self._command_stdout_can_answer_request(command=command, stdout=stdout, user_message=user_message):
                    continue
                ...
                if self._empty_stdout_command_can_answer_request(command=command, user_message=user_message):
...
def _empty_stdout_command_can_answer_request(self, *, command: str, user_message: str) -> bool:
    """Allow empty stdout only when success itself is the requested result.

    A successful command with no stdout is evidence that an action happened, not
    a user-visible answer. Promoting generic empty-output commands such as
    `ls -R` to `cwd=...` hides an incomplete task behind an environment fact.
    """
    normalized_command = str(command or "").strip().lower()
    normalized_user = str(user_message or "").lower()
    if re.fullmatch(r"(pwd|/bin/pwd|echo\s+\$pwd)", normalized_command):
        return True
    if any(marker in normalized_user for marker in ["cwd", "pwd", "現在のディレクトリ", "作業ディレクトリ", "どのディレクトリ"]):
        return True
    return False


def _command_stdout_can_answer_request(self, *, command: str, stdout: str, user_message: str) -> bool:
    """Return whether command stdout is allowed to become a final answer.

    Tool stdout is evidence by default. It becomes a user result only when the
    user asked for that command/output, or when the request explicitly requires
    running/displaying a produced artifact. Environment inspection commands are
    especially conservative because their output often looks "visible" while
    still being only a precondition observation.
    """
    if not str(stdout or "").strip():
        return False
    normalized_command = str(command or "").strip().lower()
    normalized_user = str(user_message or "").lower()
    requested_commands = [str(item).strip().lower() for item in self._extract_requested_commands(user_message)]
    if any(requested and (requested in normalized_command or normalized_command in requested) for requested in requested_commands):
        return True
    head = normalized_command.split()[0] if normalized_command.split() else ""
    if head in {"pwd", "/bin/pwd"}:
        return any(marker in normalized_user for marker in ["cwd", "pwd", "現在のディレクトリ", "作業ディレクトリ", "どのディレクトリ"])
    environment_inspection_heads = {"ls", "find", "tree"}
    if head in environment_inspection_heads:
        return any(marker in normalized_user for marker in ["ls", "一覧", "リスト", "ファイル", "ディレクトリ", "構成", "中身", "確認して"])
    artifact_result_markers = [
        "作",
        "生成",
        "実行",
        "表示",
    ...
    return any(marker in normalized_user for marker in artifact_result_markers)
