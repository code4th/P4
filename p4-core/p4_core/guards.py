...
def _failed_command_guardrail(self, *, tool_result: dict[str, Any]) -> str:
    ...
    output = str(tool_result.get("stderr") or tool_result.get("stdout") or tool_result.get("error") or "").strip()
    if output:
        lines = [line.strip() for line in output.splitlines() if line.strip()]
        traceback_lines = [
            line
            for line in lines
            if line.startswith("File ") or "Error:" in line or "Exception:" in line or line.startswith("AssertionError")
        ]
        selected = traceback_lines[-4:] if traceback_lines else lines[-4:]
        stderr_preview = " | ".join(selected)[-900:]
    else:
        stderr_preview = "no stderr/stdout"
    return (
