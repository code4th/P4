...
from p3_core.workspace import WorkspacePaths, active_session_id, read_json, read_jsonl, now_iso
...
def _trace_preview_for_operation(operation: dict[str, Any], events: list[dict[str, Any]]) -> str:
    ...
    for row in events:
        ...
        if row_type not in {"user_message", "assistant_message", "tool_call", "tool_result", "finish", "system_note", "planning_note", "observer_note", "activity_update"}: continue
...
def _flow_steps_for_operation(operation: dict[str, Any], events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    from p3_core.dashboard.templates import _phase_for_flow_step
