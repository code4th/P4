...
from p3_core import __mainline_date__, __version__
from p3_core.benchmark import run_benchmark_suite
from p3_core.dashboard.server import serve_dashboard
from p3_core.ollama_client import OllamaChatClient
from p3_core.runtime import AgentRuntime
from p3_core.workspace import DEFAULT_CONFIG, bootstrap_workspace


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="P3 Core operator CLI")
    parser.add_argument("--root", required=True, help="Path to the P3 workspace root")
    subparsers = parser.add_subparsers(dest="subcommand", required=True)

    bootstrap_parser = subparsers.add_parser("bootstrap", help="Bootstrap a new P3 workspace")
    bootstrap_parser.add_argument("--force", action="store_true")

    version_parser = subparsers.add_parser("version", help="Show P3 mainline version")
    del version_parser
    ...
    benchmark_parser = subparsers.add_parser("benchmark", help="Run a live P3 benchmark suite")
...
def main() -> None:
    ...
    if args.subcommand == "version":
        payload = {
            ...
            "canonical_handoff": "handoff/p3-canonical-mainline-2026-04-21.md",
        }
        print(json.dumps(payload, ensure_ascii=False, indent=2))
        return
    ...
    if args.subcommand == "chat":
        payload = runtime.send_message(args.message, run_immediately=args.run_immediately)
        print(json.dumps(payload, ensure_ascii=False, indent=2))
        return
    if args.subcommand == "run-loop":
        payload = runtime.run_until_idle(max_work_items=args.max_work_items)
        print(json.dumps(payload, ensure_ascii=False, indent=2))
        return
